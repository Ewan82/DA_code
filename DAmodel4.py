...
def ACM(Cf,i):
    ...
    def GPPfunc(Cf):
        GPP= 0.4*Cf**2./(0.3*Cf**2.+1.) #np.divide((d.a_7*(Cf**2)*float(d.I[i])*g_c*(d.C_a-C_i)*(d.a_2*s+d.a_5)),((d.a_7*float(d.I[i])+g_c*(d.C_a-C_i))*(Cf**2)+d.a_9*(111.**2)*g_c*(d.C_a-C_i)))        
        return GPP 

    def GPPdifffunc(Cf):		
        GPP_diff= 2.*0.4*Cf/(0.3*Cf**2.+1.)**2. #np.divide((24642.*float(d.I[i])*d.a_7*Cf*(g_c**2)*((d.C_a-C_i)**2)*(d.a_2*s+d.a_5)*d.a_9),((Cf**2+12321.*d.a_9)*(d.C_a-C_i)*g_c+float(d.I[i])*d.a_7*Cf**2)**2)      
        return GPP_diff
