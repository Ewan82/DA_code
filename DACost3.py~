...
def J(X):
    nlhx=DAm.nldalec(X,lenrun) #Run nonlinear model for length run outputing models obs guess
...
def dJ(X):
    Mlist,GPP_diff=DAm.ldalec(X,lenrun) #run linear model out list of linear model matrices and value for GPP derivative at each time step
    nlhx=DAm.nldalec(X,lenrun) #run nonlinear model for obs guess output
...
def dJ2(X):
    ...
    yhx=[y[i].T-hx[i].T for i in range(0,lenrun,1)]
    Incr=[0]*lenrun
    for i in range(lenrun):
        if np.linalg.norm(y_err[i])!=0:
            Incr[i]=np.matrix(Rmat(y_err[i])).I*yhx[i]
    #Incr=[np.matrix(Rmat(y_err[i])).I*yhx[i] for i in range(0,lenrun,1)]

    lami=[-9999]*(lenrun)

    lami[lenrun-1]=2*H[lenrun-1].T*Incr[lenrun-1]


    for i in range(lenrun-1,0,-1):
        lami[i-1]=Mlist[i-1].T*lami[i]+2*H[i-1].T*Incr[i-1]
